...
//stock market data page js code start
      ...
      // let g;  
      Promise.all([
        fetch('https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=MSFT&interval=5min&outputsize=full&apikey=XQJN6X4YXV76D14M'),
        fetch('https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=FRSH&interval=5min&outputsize=full&apikey=XQJN6X4YXV76D14M'),
        fetch('https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=AAPL&interval=5min&outputsize=full&apikey=XQJN6X4YXV76D14M')
      ...
      const labels = Object.keys(x).reverse();
            ...
            const prices2 = Object.values(y).map(item => parseFloat(item["4. close"])).reverse();
            ...
            const prices3 = Object.values(z).map(item => parseFloat(item["4. close"])).reverse();
